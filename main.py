...
async def main(page: ft.Page):
    ...
    # Проверяем, есть ли у пользователя уникальный ID в localStorage
    user_id = await page.local_storage.get("user_id")
    if not user_id:
        # Генерируем новый уникальный ID и сохраняем его в localStorage
        user_id = str(ObjectId())
        await page.local_storage.set("user_id", user_id)
    ...
    if user_data['name']:
        ...
        enter_button = ft.TextButton(text="Enter", on_click=lambda e: enter_click(e, name_input, user_id, page))
        page.add(name_input, enter_button)

    await page.update()


async def enter_click(event, name_input, user_id, page):
    # Сохраняем имя пользователя в базе данных
    save_user_name(user_id, name_input.value)
